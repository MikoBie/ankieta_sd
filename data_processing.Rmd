...
```{r get_data}
data <- read_csv("data/Ankieta dla doktorantów (Odpowiedzi) - Liczba odpowiedzi_ 1-2.csv")
...
* wyświetlić tabele z odpowiedziami na pytania otwarte. 
...
data_wide %>%
  ...
  theme(legend.position="bottom", legend.title = element_blank())
cap <- sprintf(
  "Z %i doktorantek i doktorantów, którzy wzięli udział w badaniu najwięcej reprezentowało %s - %i; %s reprezentowało %i, a %s - %i (dla 9 doktorantek i doktorantów nie udało się ustalić dziedziny nauki).",
  data_wide %>% nrow,
...
data_wide %>%
  ...
  filter(!qn_24 %in% c("Żadne zajęcia",
                       ...
                       "Brak takich zajęć")) %>%
  select("Rodzaj kształcenia" = position,
         Jednostka =  faculty_label,
         Odpowiedź = qn_24) %>%
  ungroup(id) %>%
  mutate(id = 1:n()) %>%
  kable(caption = cap, "html") %>%
...
data_wide %>%
  ...
  select(Jednostka =  faculty_label, "Rodzaj kształcenia" = position, Odpowiedź = qn_46) %>%
  ungroup(id) %>%
  mutate(id = 1:n()) %>%
  kable(caption = cap, "html") %>%
   kable_styling(bootstrap_options = c("striped","hover"),
